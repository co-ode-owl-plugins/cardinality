...
public class DataRangeFacetPanel extends JPanel {
    ...
    protected final Map<OWLFacet, JTextField> componentMap = new HashMap<>();
    ...
    private JComponent createInternalPanel() {
        ...
        for (IRI facetIRI : OWLFacet.getFacetIRIs()) {
            ...
            editor.setPreferredSize(
                    new Dimension(100, editor.getPreferredSize().height));
            gc.gridx = 1;
    ...
    public Map<OWLFacet, OWLLiteral> getFacetValueMap(OWLDatatype baseType) {
        Map<OWLFacet, OWLLiteral> results = new HashMap<>();
        ...
        for (OWLFacet facet : componentMap.keySet()) {
            ...
            if (valueStr != null && !valueStr.equals("")) {
                OWLLiteral value = df.getOWLLiteral(valueStr,
                        getOWLDataTypeForFacet(facet, baseType));
    ...
    private OWLDatatype getOWLDataTypeForFacet(OWLFacet facet,
            OWLDatatype baseType) {
        OWLDataFactory df = eKit.getModelManager().getOWLDataFactory();
        switch (facet) {
